          ...
          when HTML::Tag
            node.attributes.keys.each do |attr_name|
              ...
              if !attrs.include?(attr_name) || (protocol_attributes.include?(attr_name) && contains_bad_protocols?(value))
                ...
                # okay, this sucks, but flash sucks more
                node.attributes[attr_name] = value.gsub(/\"/n, '&quot;') # CGI::escapeHTML(value)
              end
            end if node.attributes
            node.attributes.update({'rel' => 'nofollow'}) if nofollow && node.name == 'a' && node.attributes
            if tags.include?(node.name)
              bad = nil
            else
              bad = node.name
              node = block.call node, bad
            end
