  ...
  module Acts #:nodoc:
    module Taggable #:nodoc:
      ...
      module InstanceMethods
        ...
        # update tags & cached tags list without calling self.save. this is much faster in some situations
        def update_tags
          self.class.transaction do
            self.save_tags && \
            self.connection.update("UPDATE #{self.class.table_name} SET #{self.class.cached_tag_list_column_name} = '#{self.connection.quote_string(self.tag_list.to_s)}' WHERE id = #{self.id}")
          end
        end
